...
let tplCompile = (name, item, tmpfileName) => {
    var source = fs.readFileSync(path.join(process.cwd(), CONF.TPL_DIST, `${name === 'router' && tmpfileName === 'index' ? 'apiIndex' : name}.hbs`),'utf-8');
    ...
    if(name === 'api' && tmpfileName) {
        console.log(path.join(process.cwd(), CONF.TPL_DIST, `${name}.hbs`));
        console.log(fileFullName)
    };
    FILE_OPERATES.write(result, fileFullName, `${name}.${item.sClassName}`)
}
