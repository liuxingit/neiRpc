...
async function getRPCS(){
    ...
    rpcs.map(function(item){
        ...
        const patternGroup = new RegExp('\\b' + groupName + '\\b')
        if(!(CONF.GROUP_NAME || []).length || patternGroup.test(CONF.GROUP_NAME)){
    ...
    let st = setInterval(function(){
        if(rpcs.length === ajaxCount){
            ...
            AllClassNamePathArr.map(item => {
                item.paths.map(item2 => {
                    (item2.params || []).map((param) => {
                        ...
                        if(CONF.TYPE_SWITCH.indexOf(typeName) > -1){
            ...
            FILE_OPERATES.write(JSON.stringify(AllClassNamePathArr, null, 4), fileFullName, `mock数据`)
